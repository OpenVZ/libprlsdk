...
/**
* Tar file descriptor type
*/
typedef void* tarfd_t;

#define TAR_INVALID_FD (((char*)0) - 1)

/**
* Callback function to check quit
*/
typedef int (*PACK_OPERATION_CHECKQUIT_CALLBACK) (void*);

/***** handle.c ************************************************************/

typedef tarfd_t(*openfunc_t)(const char *, int, int);
typedef int(*closefunc_t)(tarfd_t);
typedef ssize_t(*readfunc_t)(tarfd_t, void *, size_t);
typedef ssize_t(*writefunc_t)(tarfd_t, const void *, size_t);
...
int tar_fdopen(TAR **t, tarfd_t fd, char *pathname, tartype_t *type,
	       int oflags, int mode, int options);

/* returns the descriptor associated with t */
tarfd_t tar_fd(TAR *t);
...
int tar_append_file(TAR *t, char *realname, char *savename,
						PACK_OPERATION_CHECKQUIT_CALLBACK pFunc, void* pParam );

/* write EOF indicator */
int tar_append_eof(TAR *t);

/* add file contents to a tarchive */
int tar_append_regfile(TAR *t, char *realname, PACK_OPERATION_CHECKQUIT_CALLBACK pFunc, void* pParam);
