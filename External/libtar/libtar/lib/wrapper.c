...
int
tar_append_tree(TAR *t, char *realdir, char *savedir, PACK_OPERATION_CHECKQUIT_CALLBACK pFunc, void* pParam )
{
	...
	if (tar_append_file(t, realdir, savedir, pFunc, pParam ) != 0)
		return -1;
	...
	while ((dent = readdir(dp)) != NULL)
	{
		...
		if (S_ISDIR(s.st_mode))
		{
			if (tar_append_tree(t, realpath,
					    (savedir ? savepath : NULL), pFunc, pParam ) != 0)
				return -1;
			continue;
		}
