...
namespace
{
    ...
    CVmDevice* getDeviceFromString(uint devType, const QString& xmlCfg)
    {
        if ( xmlCfg.isEmpty() )
            return 0;
        ...
        switch( devType )
        {
            ...
            {
                CVmFloppyDisk* floppyDisk = new CVmFloppyDisk();
                StringToElement<CVmFloppyDisk*>( floppyDisk, xmlCfg );
                return floppyDisk;
            }
            case PDE_SOUND_DEVICE:
            {
                CVmSoundDevice* sound = new CVmSoundDevice();
                StringToElement<CVmSoundDevice*>( sound, xmlCfg );
                return sound;
            }
            case PDE_USB_DEVICE:
            {
                CVmUsbDevice* usb = new CVmUsbDevice();
                StringToElement<CVmUsbDevice*>( usb, xmlCfg );
                return usb;
            }
            case PDE_OPTICAL_DISK:
            {
                CVmOpticalDisk* optDisk = new CVmOpticalDisk();
                StringToElement<CVmOpticalDisk*>( optDisk, xmlCfg );
                return optDisk;
            }
            case PDE_HARD_DISK:
            {
                CVmHardDisk* hardDisk = new CVmHardDisk();
                StringToElement<CVmHardDisk*>( hardDisk, xmlCfg );
                return hardDisk;
            }
            case PDE_SERIAL_PORT:
            {
                CVmSerialPort* serialPort = new CVmSerialPort();
                StringToElement<CVmSerialPort*>( serialPort, xmlCfg );
                return serialPort;
            }
            case PDE_PARALLEL_PORT:
            {
                CVmParallelPort* parallelPort = new CVmParallelPort();
                StringToElement<CVmParallelPort*>( parallelPort, xmlCfg );
                return parallelPort;
            }
            case PDE_GENERIC_NETWORK_ADAPTER:
            {
                CVmGenericNetworkAdapter* nAdapter = new CVmGenericNetworkAdapter();
                StringToElement<CVmGenericNetworkAdapter*>( nAdapter, xmlCfg );
                return nAdapter;
            }
            default:
                return 0;
