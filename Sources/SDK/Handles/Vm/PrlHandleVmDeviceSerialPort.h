...
#ifndef __PARALLELS_HANDLE_VM_DEVICE_SERIAL_PORT_H__
#define __PARALLELS_HANDLE_VM_DEVICE_SERIAL_PORT_H__


#include "PrlHandleVmDevice.h"

/**
 * Serial port VM device handle object.
 */
class PrlHandleVmDeviceSerialPort : public PrlHandleVmDevice
{
...
public:
	 ...
	 */
	PrlHandleVmDeviceSerialPort( const PrlHandleVmPtr &pVm, class CVmSerialPort *pSerial = 0 );
...
public://SDK API helpers set
	...
	/**
	 * Gets serial port socket mode.
	 *
	 * @param buffer for storing result
	 * @return PRL_RESULT
	 */
	PRL_RESULT GetSocketMode(PRL_SERIAL_PORT_SOCKET_OPERATION_MODE_PTR pnSocketMode);

	/**
	 * Sets serial port socket mode.
	 *
	 * @param assigning value
	 * @return PRL_RESULT
	 */
	PRL_RESULT SetSocketMode(PRL_SERIAL_PORT_SOCKET_OPERATION_MODE nSocketMode);

private:

	/// Storing VM floppy XML representation.
	class CVmSerialPort *m_pVmSerialPort;
...
private:
	...
	/**
	 * Overridden method that returns pointer to VM port device XML model object
	 * @param return code that should be correctly filled
	 */
	class CVmPort *GetVmPortDeviceElem(PRL_RESULT &nRetCode);
...
typedef PrlHandleSmartPtr<PrlHandleVmDeviceSerialPort> PrlHandleVmDeviceSerialPortPtr;

#endif // __PARALLELS_HANDLE_VM_DEVICE_SERIAL_PORT_H__
