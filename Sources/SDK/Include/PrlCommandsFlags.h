...
/* VM start mode:
	...
	PSM_VM_START_FOR_REPAIR  : start VM for manual repair
*/
typedef enum _PRL_VM_START_MODE
{
	...
	PSM_VM_START_FOR_REPAIR			= 1<<(PACF_MAX+4),
	// PSM_VM_START_LAST_ITEM has to be equal the last mode
	PSM_VM_START_LAST_ITEM	= PSM_VM_START_FOR_REPAIR,
} PRL_VM_START_MODE;
...
typedef enum _PRL_GET_VM_LIST_FLAGS
{
	...
	PGVLF_GET_NET_INFO		= 1 << (PACF_MAX+7),
	PGVLF_GET_NET_STATIC_IP_INFO	= 1 << (PACF_MAX+8),
